...
const HelloWorldApp = () => {
  ...
  const getUserData = async () => {
      const response = await fetch(`http://192.168.56.1/OnRoadHelp1/api/Help/Aservices?id=1`);
...
//end
  ...
  return (
    ...
    borderWidth:2, margin:5}}>
        ...
        <Text style={{fontSize:16,fontWeight:"bold"}}>Service_Name:{item.Service_Name}</Text> 
        {/* <Text style={{fontSize:16,fontWeight:"bold"}}>Service_Type:{item.Service_Type}</Text> */}
        <Text style={{fontSize:16,fontweight:"bold"}}>count:{item.count}</Text>
