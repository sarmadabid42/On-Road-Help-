...
const HelloWorldApp = (props) => {
  ...
  const getUserData = async () => {
      const response = await fetch(`http://192.168.56.1/OnRoadHelp1/api/Help/GetList?id=`+global.userID);
...
//end
  ...
  return (
    ...
    borderWidth:2, margin:5}}>
        ...
        <Text style={{fontSize:22,fontWeight:"bold",marginLeft:10}}>First Name: {item.First_Name}</Text> 
        {/* <Text style={{fontSize:16,fontWeight:"bold"}}>Service_Type:{item.Service_Type}</Text> */}
        <Text style={{fontSize:20,fontweight:"bold",marginLeft:10}}>Last Name: {item.Last_Name}</Text>
        {/* <Text style={{fontSize:20,fontweight:"bold",marginLeft:10}}>Status: {item.loginstatus}</Text> */}
